...
void CiCalExporter::ExportTask(const ITaskList* pSrcTaskFile, HTASKITEM hTask, CStdioFile& fileOut)
{
	...
	if (tStart && !pSrcTaskFile->IsTaskDone(hTask))
	{
		// header
//		WriteString(fileOut, "BEGIN:VTODO");
		...
		date = tStart;
		WriteString(fileOut, "DTSTART:%04d%02d%02d", date.GetYear(), date.GetMonth(), date.GetDay());
		...
		if (!tDate || tDate < tStart)
			tDate = tStart + ONEDAY;
		
		date = tDate;
		WriteString(fileOut, "DUE:%04d%02d%02d", date.GetYear(), date.GetMonth(), date.GetDay());
		WriteString(fileOut, "DTEND:%04d%02d%02d", date.GetYear(), date.GetMonth(), date.GetDay());
