...
void CColourPicker::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	COLORREF color = (m_crColourBk == CLR_DEFAULT || !bEnabled) ? crWindow : m_crColourBk;
	
    GetWindowText(m_strText);

	if (m_nSelectionMode == CP_MODE_BK)
	{
		// draw border in WINDOW_COLOR
		pDC->Draw3dRect(rect, crWindow, crWindow);
	...
	else if (m_strText.GetLength())
    {
		...
		COLORREF crText = !bEnabled ? GetSysColor(COLOR_3DSHADOW) : 
							((m_crColourText == CLR_DEFAULT) ? ::GetSysColor(COLOR_WINDOWTEXT) : m_crColourText);
