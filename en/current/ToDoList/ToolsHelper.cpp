...
CToolsHelper::CToolsHelper(UINT nStart, int nSize) : m_nStartID(nStart), m_nSize(nSize)
{
...
BOOL CToolsHelper::PrepareCmdline(const USERTOOL& tool, CString& sCmdline, const USERTOOLARGS& args)
{
	...
	{
		char szToDoList[MAX_PATH + 1];
		::GetModuleFileName(NULL, szToDoList, MAX_PATH);
		
		tcp.ReplaceArgument(CLAT_TODOLIST, szToDoList);
	}
...
void CToolsHelper::AppendToolsToToolbar(const CUserToolArray& aTools, CToolBar& toolbar, UINT nCmdAfter, CToolbarHelper* pTBHelper)
{
	// remove tools first
...
LPCTSTR CToolsHelper::GetFileFilter()
{
	...
	if (CPreferencesGenPage().GetEnableTDLExtension())
