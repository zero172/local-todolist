...
BOOL CIni::AppendString(LPCTSTR lpSection, LPCTSTR lpKey, LPCTSTR lpString) const
{
	...
	//fabio_2005
#if _MSC_VER >= 1400
	_stprintf_s(pNewString,_tcslen(psz) + _tcslen(lpString) + 1, _T("%s%s"), psz, lpString);
#else
...
BOOL CIni::WriteDouble(LPCTSTR lpSection, LPCTSTR lpKey, double fValue, int nPrecision) const
{
	...
	if (nPrecision > 0)
		//fabio_2005
#if _MSC_VER >= 1400
		_stprintf_s(szFmt, _T("%%.%df"), nPrecision);
#else
	...
	//fabio_2005
#if _MSC_VER >= 1400
	_stprintf_s(szValue, szFmt, fValue);
#else
...
void CIni::__IntToString(int nNumber, LPTSTR lpBuffer, int nBase)
{
	...
	default:
		//fabio_2005
#if _MSC_VER >= 1400
		_stprintf_s(lpBuffer,64, _T("%d"), nNumber);
#else
...
void CIni::__UIntToString(UINT nNumber, LPTSTR lpBuffer, int nBase)
{
	...
	case BASE_OCTAL:
#if _MSC_VER >= 1400
		_stprintf_s(lpBuffer,64, _T("%o"), nNumber);
#else
	...
	case BASE_HEXADECIMAL:
#if _MSC_VER >= 1400
		_stprintf_s(lpBuffer,64, _T("%X"), nNumber);
#else
