...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
          ...
          .progressBar {
			...
			<xsl:element name="body">
				...
				<xsl:element name="div">
					...
					<xsl:element name="h2">项目名称</xsl:element>
				...
				<xsl:element name="div">
					...
					<xsl:element name="h5">由TodoList创建 - http://www.abstractspoon.com<xsl:element name="br"/>使用TodoListStyler v2.0</xsl:element>
				</xsl:element>
			</xsl:element>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="get_Task_Base_Info">
		<xsl:element name="span">
			...
			<xsl:element name="a">[任务ID: <xsl:value-of select="@ID" />]</xsl:element>
			...
			<xsl:choose>
				<xsl:when test="@STATUS">
					<xsl:element name="a">[状态: <xsl:value-of select="@STATUS" />]</xsl:element>
				</xsl:when>
				<xsl:otherwise>
					<xsl:element name="a">[状态: 无]</xsl:element>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="get_Pretty_Percent_Bar">
		<xsl:element name="span">[完成: <xsl:value-of select="@CALCPERCENTDONE" />% ]</xsl:element>
	...
	<xsl:template name="get_Pretty_Priority">
		<xsl:element name="span">
			<xsl:element name="span">[优先级: </xsl:element>
			<xsl:element name="span">
		...
		<xsl:element name="span">
			...
			<xsl:choose>
				<xsl:when test="@STARTDATESTRING">
					<xsl:element name="a">[开始时间: <xsl:value-of select="@STARTDATESTRING" />]</xsl:element>
				</xsl:when>
				<xsl:otherwise>
					<xsl:element name="a">[开始时间: 无]</xsl:element>
				</xsl:otherwise>
			</xsl:choose>
			...
			<xsl:choose>
				<xsl:when test="@DUEDATESTRING">
					<xsl:element name="a">[到期时间: <xsl:value-of select="@DUEDATESTRING" />]</xsl:element>
				</xsl:when>
				<xsl:otherwise>
					<xsl:element name="a">[到期时间: 无]</xsl:element>
				</xsl:otherwise>
			</xsl:choose>
			...
			<xsl:choose>
				<xsl:when test="@DONEDATESTRING">
					<xsl:element name="a">[已完成: <xsl:value-of select="@DONEDATESTRING" />]</xsl:element>
				</xsl:when>
				<xsl:otherwise>
					<xsl:element name="a">[已完成: 无]</xsl:element>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:element>
	</xsl:template>
		...
		<xsl:element name="span">
			...
			<xsl:choose>
				<xsl:when test="@PERSON">
					<xsl:element name="a">[接收人: <xsl:value-of select="@PERSON" />]</xsl:element>
				</xsl:when>
				<xsl:otherwise>
					<xsl:element name="a">[接收人: 无]</xsl:element>
				</xsl:otherwise>
			</xsl:choose>
			...
			<xsl:choose>
				<xsl:when test="@ALLOCATEDBY">
					<xsl:element name="a">[指定人: <xsl:value-of select="@ALLOCATEDBY" />]</xsl:element>
				</xsl:when>
				<xsl:otherwise>
					<xsl:element name="a">[指定人: 无]</xsl:element>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="get_Task_Fileref">
		<xsl:element name="span">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:element name="a">[参考文件: 无]</xsl:element>
				</xsl:otherwise>
			</xsl:choose>
	</xsl:element>
	</xsl:template>
	...
	<xsl:template name="get_Task_Comment">
      ...
      <xsl:choose>
				...
				<xsl:otherwise>
					<xsl:element name="a">这个任务没有备注</xsl:element>
				</xsl:otherwise>
			</xsl:choose>
    </xsl:element>
	</xsl:template>
