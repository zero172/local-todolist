...
CWndPrompt::CWndPrompt() : m_nCheckMsg(0), m_lCheckResult(0)
{
...
BOOL CWndPrompt::Initialize(HWND hWnd, LPCTSTR szPrompt, UINT nCheckMsg, LRESULT lRes)
{
	ASSERT (hWnd);
	ASSERT (!IsHooked());
...
BOOL CWndPromptManager::SetPrompt(HWND hWnd, LPCTSTR szPrompt, UINT nCheckMsg, LRESULT lRes)
{
	...
	if (pWnd && pWnd->Initialize(hWnd, szPrompt, nCheckMsg, lRes))
	{
		m_mapWnds[hWnd] = pWnd;
		return TRUE;
	}
...
BOOL CWndPromptManager::SetPrompt(HWND hWnd, UINT nIDPrompt, UINT nCheckMsg, LRESULT lRes)
{
	return SetPrompt(hWnd, CEnString(nIDPrompt), nCheckMsg, lRes);
}

BOOL CWndPromptManager::SetPrompt(UINT nIDCtrl, HWND hwndParent, LPCTSTR szPrompt, UINT nCheckMsg, LRESULT lRes)
{
	return SetPrompt(GetDlgItem(hwndParent, nIDCtrl), szPrompt, nCheckMsg, lRes);
}

BOOL CWndPromptManager::SetPrompt(UINT nIDCtrl, HWND hwndParent, UINT nIDPrompt, UINT nCheckMsg, LRESULT lRes)
{
	return SetPrompt(nIDCtrl, hwndParent, CEnString(nIDPrompt), nCheckMsg, lRes);
}
